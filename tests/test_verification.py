...
class TestDocumentTypeMismatch(unittest.TestCase):
    """
    REGRESSION — Bug 3: Verify KNOWN_CONFLICT_MAP contains cross-matrix conflicts
    between identity types (e.g. NATIONAL_ID vs VISA, DRIVING_LICENSE vs NATIONAL_ID, etc.).
    """

    def test_aadhaar_as_visa_rejected(self):
        from app.services.verification import KNOWN_CONFLICT_MAP
        self.assertIn("NATIONAL_ID", KNOWN_CONFLICT_MAP["VISA"])

    def test_driving_license_as_national_id_rejected(self):
        from app.services.verification import KNOWN_CONFLICT_MAP
        self.assertIn("DRIVING_LICENSE", KNOWN_CONFLICT_MAP["NATIONAL_ID"])

    def test_permit_as_passport_rejected(self):
        from app.services.verification import KNOWN_CONFLICT_MAP
        self.assertIn("PERMIT", KNOWN_CONFLICT_MAP["PASSPORT"])

    def test_passport_as_permit_rejected(self):
        from app.services.verification import KNOWN_CONFLICT_MAP
        self.assertIn("PASSPORT", KNOWN_CONFLICT_MAP["PERMIT"])

    def test_document_validation_conflict_set(self):
        from app.services.document_validation import _KNOWN_CONFLICT_MAP, validate_document
        self.assertIn("NATIONAL_ID", _KNOWN_CONFLICT_MAP["VISA"])
        val = validate_document("VISA", {}, detected_doc_type="NATIONAL_ID")
        self.assertFalse(val.format_valid)
        self.assertTrue(any("MISMATCH" in issue for issue in val.issues))
