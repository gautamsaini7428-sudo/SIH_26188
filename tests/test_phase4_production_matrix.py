...
def test_matrix_case_a_valid_matching_document():
    ...
    with TestClient(app) as client:
        ...
        data = res.json()
        assert data["verdict"] in ("GENUINE", "SUSPICIOUS", "FAKE", "REJECTED")
...
def test_matrix_case_d2_aadhaar_as_visa_rejected():
    """
    REGRESSION — Bug 3: Upload an Aadhaar card while declaring document_type=VISA.
    Because KNOWN_CONFLICT_MAP['VISA'] includes NATIONAL_ID, this must be flagged
    as category_match=False and verdict=REJECTED with reason mentioning MISMATCH.
    """
    with TestClient(app) as client:
        token = make_officer_token()
        img = Image.new("RGB", (700, 450), color=(245, 245, 245))
        draw = ImageDraw.Draw(img)
        draw.rectangle([0, 0, 700, 60], fill=(30, 45, 35))
        draw.text((30, 20), "GOVERNMENT OF INDIA", fill=(255, 255, 255))
        draw.rectangle([30, 90, 200, 310], fill=(210, 170, 140))
        draw.text((240, 100), "AADHAAR CARD", fill=(0, 0, 0))
        draw.text((240, 140), "UID: 9876 5432 1098", fill=(0, 0, 0))
        draw.text((240, 180), "NAME: RAJESH KUMAR", fill=(0, 0, 0))
        draw.text((240, 220), "DOB: 12/05/1985", fill=(0, 0, 0))
        buf = io.BytesIO()
        img.save(buf, format="JPEG")
        doc_bytes = buf.getvalue()
        ...
        res = client.post(
            "/verify-document",
            files={"file": ("aadhaar_as_visa.jpg", doc_bytes, "image/jpeg")},
            data={"document_type": "VISA"},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert res.status_code == 200
        data = res.json()
        assert data["category_match"] is False
        assert data["verdict"] == "REJECTED"
        assert "MISMATCH" in data["reason"].upper()
