...
def extract_gender(text: str, lines: Optional[List[Dict[str, Any]]] = None) -> Optional[str]:
    """
    Extracts gender/sex if present. Tolerates OCR noise (e.g. 'Se x', 'MAI E', 'FEMAIE')
    and scopes single-letter (M/F) or standalone matches to identity context lines.
    """
    if not text:
        return None

    # 1. Broad labeled pattern with OCR noise tolerance
    label_pattern = r'\b(?:G[E36]NDER|G\s+ENDER|SEX|5EX|S\s*e\s*x)\b[\s:/-]*\b(MALE|MAI\s*E|M\s*ALE|MA\s*LE|NALE|FEMALE|FEMAIE|FEM\s*ALE|FEMAL\s*E|TRANSGENDER|M|F)\b'
    m = re.search(label_pattern, text, re.IGNORECASE)
    if m:
        raw_val = re.sub(r'\s+', '', m.group(1).upper())
        if raw_val in ("M", "MALE", "MAIE", "NALE"):
            return "MALE"
        if raw_val in ("F", "FEMALE", "FEMAIE"):
            return "FEMALE"
        if raw_val == "TRANSGENDER":
            return "TRANSGENDER"

    # Split text into lines for line-scoped analysis
    raw_lines = text.splitlines()
    if lines:
        line_texts = [l.get("text", "") if isinstance(l, dict) else str(l) for l in lines]
        all_lines = [lt.strip() for lt in line_texts if str(lt).strip()]
        if not all_lines:
            all_lines = [rl.strip() for rl in raw_lines if rl.strip()]
    else:
        all_lines = [rl.strip() for rl in raw_lines if rl.strip()]

    # Context patterns indicating identity/demographic field line
    context_pattern = r'\b(?:G[E36]NDER|SEX|S\s*e\s*x|DOB|BIRTH|DATE|NAME|ID|LICENCE|LICENSE|FATHER|MOTHER|HUSBAND)\b'
    date_pattern = r'\b\d{2}[-/.]\d{2}[-/.]\d{4}\b|\b\d{4}[-/.]\d{2}[-/.]\d{2}\b'

    for line in all_lines:
        has_context = bool(re.search(context_pattern, line, re.IGNORECASE) or re.search(date_pattern, line))

        if has_context:
            if re.search(r'\b(?:FEMALE|FEMAIE|FEM\s*ALE)\b', line, re.IGNORECASE):
                return "FEMALE"
            if re.search(r'\b(?:MALE|MAI\s*E|M\s*ALE|MA\s*LE)\b', line, re.IGNORECASE):
                return "MALE"

            # Isolated single letter M or F on a context line
            m_single = re.search(r'\b(M|F)\b', line)
            if m_single:
                val = m_single.group(1).upper()
                if val == "M":
                    return "MALE"
                if val == "F":
                    return "FEMALE"

    # Fallback: standalone MALE/FEMALE on lines that do NOT contain address/location tokens
    address_pattern = r'\b(?:ROAD|STREET|AVENUE|COLONY|NAGAR|VILLAGE|DISTRICT|POST|PIN|STATE|HOSTEL|BUILDING|HOUSE|FLAT|APT)\b'
    for line in all_lines:
        if re.search(address_pattern, line, re.IGNORECASE):
            continue
        if re.search(r'\b(?:FEMALE|FEMAIE)\b', line, re.IGNORECASE):
            return "FEMALE"
        if re.search(r'\b(?:MALE|MAI\s*E|M\s*ALE)\b', line, re.IGNORECASE):
            return "MALE"
