    ...
    try:
        ...
        for idx, image in enumerate(images):
            ...
            if "ocr_status" in result:
                last_ocr_status = result["ocr_status"]
        ...
        response = OCRResponse(
            ...
            ocr_status=last_ocr_status,
        )
        return response
    except OCREngineUnavailableError as oe:
        logger.error(f"OCR engine unavailable: {oe}")
        return OCRResponse(
            raw_text="",
            lines=[],
            fields=ExtractedFields(),
            mrz=MRZResult(detected=False, raw=None, valid=None, fields={}),
