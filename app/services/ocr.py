...
from app.engines import get_ocr_engine, OCREngineUnavailableError
    ...
    for variant_name, var_img in variants:
        try:
            ...
            if raw_lines:
                ...
                # Check cumulative merged lines after every pass: if >= 8 lines found, stop early to save latency
                merged_so_far = _merge_raw_ocr_lines(all_passes_lines)
                if len(merged_so_far) >= 8:
                    logger.debug(f"OCR early exit triggered after variant '{variant_name}' ({len(merged_so_far)} lines merged)")
                    break
        except OCREngineUnavailableError:
            # Hard failure — the engine itself is broken; propagate immediately
            # so extract_fields() can surface OCR_ENGINE_UNAVAILABLE
            raise
        except Exception as e:
...
async def extract_fields(file_path: str, doc_type: str = "DRIVING_LICENSE") -> Dict[str, Any]:
    ...
    if os.path.exists(file_path):
        try:
            ...
            logger.error(
                f"OCR engine unavailable for {file_path}: {e}. "
                f"Install 'easyocr + torch' or 'paddleocr' to enable text extraction."
            )
    ...
    if doc_type_upper == "PASSPORT":
        ...
        if nat:
            fields["nationality"] = nat
        elif doc_type_upper in ("NATIONAL_ID", "DRIVING_LICENSE", "VOTER_ID", "PAN", "AADHAAR"):
            fields["nationality"] = "INDIAN"
            fields["nationality_source"] = "inferred"
