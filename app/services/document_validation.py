...
# Full cross-matrix: every ID type conflicts with every other ID type.
# Prevents Aadhaar (NATIONAL_ID) slipping through when VISA is selected, etc.
_ALL_ID_TYPES_VALIDATION = {"PASSPORT", "DRIVING_LICENSE", "NATIONAL_ID", "VISA", "PERMIT"}
_NON_ID_TYPES_VALIDATION = {"INVOICE", "CERTIFICATE"}
_KNOWN_CONFLICT_MAP = {
    doc: (_ALL_ID_TYPES_VALIDATION - {doc}) | _NON_ID_TYPES_VALIDATION
    for doc in _ALL_ID_TYPES_VALIDATION
}
