...
def run_copy_move_detection(img: Image.Image) -> Tuple[int, List[Tuple[int, int, int, int]]]:
    """
    Fast block-based copy-move / patch cloning detection across image sectors.
    Uses resolution-adaptive downsampling for high-speed analysis.
    """
    ...
    if max(orig_w, orig_h) > max_dim:
        ...
        new_w = max(32, int(orig_w * scale_factor))
        new_h = max(32, int(orig_h * scale_factor))
        gray_small = img.convert("L").resize((new_w, new_h), Image.Resampling.BILINEAR)
    else:
    ...
    for y in range(0, height - block_size, step):
        for x in range(0, width - block_size, step):
            crop = gray_small.crop((x, y, x + block_size, y + block_size))
            stat = ImageStat.Stat(crop)
            # Signature: mean, stddev
        ...
        if len(coords) >= 2:
            for i in range(len(coords)):
                for j in range(i + 1, min(len(coords), i + 4)):
                    ...
                    dist = (dx**2 + dy**2)**0.5
                    if dist > 50:  # Non-adjacent duplicate in scaled coords
                        clone_pairs += 1
                        orig_x = int(coords[i][0] * inv_scale)
                        orig_y = int(coords[i][1] * inv_scale)
                        orig_bs = int(block_size * inv_scale)
                        matched_regions.append((orig_x, orig_y, orig_bs, orig_bs))
...
def _detect_tampering_sync(file_path: str) -> TamperingAnalysisResult:
...
async def detect_tampering(file_path: str) -> TamperingAnalysisResult:
    ...
    Offloads CPU image calculations to a background thread pool.
    """
    return await asyncio.to_thread(_detect_tampering_sync, file_path)