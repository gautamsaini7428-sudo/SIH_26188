...
export const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  const nationalitySource = extracted_fields.nationality_source
  const nationality = extracted_fields.nationality
    ? nationalitySource === 'inferred'
      ? `${extracted_fields.nationality} (Inferred)`
          ...
          {document_type !== 'VISA' && (
            <Card className="border-[#E5DDD8] bg-[#FFFFFF] p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                  ...
                  <div className="p-3 rounded-lg border border-[#E5DDD8] bg-[#FCFAF8] space-y-1">
                    ...
                    <span className={`font-bold text-sm ${biometric.liveness.status === 'PASS' ? 'text-[#0B2925]' : biometric.liveness.status === 'FAIL' ? 'text-[#DC2626]' : 'text-[#755B73]'}`}>
                      {biometric.liveness.status} ({Math.round(biometric.liveness.score > 1 ? biometric.liveness.score : biometric.liveness.score * 100)}/100)
                    </span>
                  </div>
              ...
              <div className="p-3 rounded-lg border border-[#E5DDD8] bg-[#FCFAF8] flex items-center justify-between text-xs font-mono">
                <span className="text-[#755B73]">ArcFace Correlation Score:</span>
